...
StatusType await_periodic_dispatch(thread_config_t * config)
{
  ...
  int sys_ret;
  struct timespec end;
  const unsigned int period = info -> period;
  unsigned long iteration_counter = info -> iteration_counter++;

  sys_ret = clock_gettime( CLOCK_MONOTONIC, &end);
  if (sys_ret != 0)
	{
#ifdef DEBUG
	printf("Error: clock get time %d\n", sys_ret);
#endif
	  return sys_ret;
	}

   end.tv_nsec += period * 1000000 * iteration_counter;
