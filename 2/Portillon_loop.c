/********
* ec2c version 0.67
* c main file generated for node : Portillon 
...
/* Output procedures **********************/
#ifdef CKCHECK
void Portillon_BOT_alarm(void* cdata){
   _put_bottom("alarm");
}
#endif
/* Output procedures **********************/
void Portillon_O_alarm(void* cdata, _boolean _V) {
   _put_bool("alarm", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct Portillon_ctx* ctx = Portillon_new_ctx(NULL);
   Portillon_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
      ...
      fflush(stdout);
      ++s;
      Portillon_I_A(ctx, _get_bool("A"));
      Portillon_I_B(ctx, _get_bool("B"));
      Portillon_step(ctx);
